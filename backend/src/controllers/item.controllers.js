...
const createItem = async (req, res) => {
  ...
  try {
    ...
    return res
    .status(500)
    .json(new ApiResponse(500, error.message, null));
  }
};
...
const getItem = async (req, res) => {
    ...
    try {
        ...
        if (!item) {
            return res
                .status(404)
                .json(new ApiResponse(404, "Item not found", null));
        }
        return res
            .status(200)
...
const updateItem = async (req, res) => {
    ...
    if(item.Seller !== req.user?._id) {
        return res
            .status(403)
            .json(new ApiResponse(403, "You are not authorized to update this item", null));
    }
...
const deleteItem = async (req, res) => {
    ...
    try {
        ...
        if(item.Seller !== req.user?._id) {
            return res
                .status(403)
                .json(new ApiResponse(403, "You are not authorized to delete this item", null));
        }
...
export { createItem, getItems, getItem, updateItem, deleteItem, getMyItem , searchItem };
